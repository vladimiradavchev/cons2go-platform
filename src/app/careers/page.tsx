...
import { getAllJobs, getSiteContent } from "@/lib/cms";
import type { JobPosting, Benefit } from "@/types/cms";
...
export default async function CareersPage() {
  ...
  const benefits: Benefit[] = content?.careerBenefits || defaultBenefits;
  const heroTitle = content?.careersHeroTitle || "Help us democratize consulting";
  const heroDesc = content?.careersHeroDesc || "We're a growing team building the global marketplace for expertise. Join us and shape how millions of businesses find answers.";
        ...
        <div className="mx-auto max-w-7xl px-6 py-20 md:py-28">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="mt-3 text-3xl font-extrabold md:text-5xl">
              {heroTitle.includes("<span") ? (
                <span dangerouslySetInnerHTML={{ __html: heroTitle }} />
              ) : (
                <>
                  {heroTitle.split("democratize")[0]}
                  {" "}
          ...
          <div className="mx-auto max-w-7xl px-6 py-20">
            ...
            <div className="flex flex-col gap-4">
              {jobs.map((j: JobPosting) => (
                <div key={j._id} className="group flex flex-col md:flex-row md:items-center justify-between gap-4 rounded-2xl border border-[var(--border)] bg-[var(--background)] p-6 transition-all hover:border-[var(--primary)] hover:shadow-lg cursor-pointer">
